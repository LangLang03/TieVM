...
StatusOr<Value> VmThread::ExecuteFunction(
    ...
    auto vm_frame_for_pc = [&](uint32_t frame_pc) {
        ...
        for (const auto& debug : module.debug_lines()) {
            if (debug.function_index == function_index &&
                debug.instruction_index == frame_pc) {
                frame.line = debug.line;
                frame.column = debug.column;
                break;
            }
        }
        return frame;
            ...
            switch (inst.opcode) {
                ...
                case OpCode::kFfiCall: {
                    ...
                    if (!ffi_or.ok() && ffi_or.status().code() == ErrorCode::kNotFound) {
                        const auto loaded_names = owner_->loader().ActiveModuleNames();
                        for (const auto& loaded_name : loaded_names) {
                            auto loaded_or = owner_->loader().GetModule(loaded_name);
                            if (!loaded_or.ok()) {
                                continue;
                            }
                            const auto& loaded = loaded_or.value();
                            bool has_binding = false;
                            for (const auto& binding : loaded.ffi_bindings()) {
                            ...
                            if (ffi_or.ok() ||
                                ffi_or.status().code() != ErrorCode::kNotFound) {
                                break;
                            }
                        }
                    }
