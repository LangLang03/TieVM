...
- LLVM AOT 编译链路：`tiebc aot` 可将 `.tbc/.tlb/.tlbs` 编译为本地可执行文件，或基于导出函数生成 `.so/.dll/.dylib` 与对应头文件。
...
- `reg_count`
- `param_count`
- `param_types`（固定参数类型签名，`any/null/int64/float64/bool/object/pointer/string/closure`）
- `return_type`（函数返回类型签名，`any/null/int64/float64/bool/object/pointer/string/closure`）
- `upvalue_count`
- `is_vararg`
- `is_exported`
- （可选）FFI 绑定头
